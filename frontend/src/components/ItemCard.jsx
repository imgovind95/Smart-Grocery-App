...
import api from '../services/api'; // Import api to call pantry endpoint

const ItemCard = ({ item }) => {
  const { addToCart, cart } = useCart();
  const navigate = useNavigate();
  const [isAddedToCart, setIsAddedToCart] = useState(false);
  const [isAddingToPantry, setIsAddingToPantry] = useState(false); // State for pantry button
  ...
  return (
    <div className="bg-white dark:bg-gray-800 border dark:border-gray-700 rounded-lg shadow-lg overflow-hidden flex flex-col justify-between transform transition-transform hover:scale-105">
      ...
      <div className="p-4 border-t border-gray-200 dark:border-gray-700 space-y-2">
         ...
         <button
            ...
            className={`w-full py-2 px-4 rounded-lg font-semibold text-sm transition-colors duration-300 ${
                isAddingToPantry
