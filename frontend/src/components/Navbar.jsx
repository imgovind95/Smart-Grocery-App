...
const Navbar = () => {
  ...
  // Refs banaye taaki hum detect kar sakein click kaha hua
  const menuRef = useRef(null);
  const buttonRef = useRef(null);

  // Scroll effect (Navbar hide/show + Close menu on scroll)
  useEffect(() => {
    const controlNavbar = () => {
      // Jaise hi scroll ho, menu band kar do
      if (isMobileMenuOpen) {
    ...
    window.addEventListener('scroll', controlNavbar);
    return () => window.removeEventListener('scroll', controlNavbar);
  }, [lastScrollY, isMobileMenuOpen]); // Dependencies update kiye

  // Click Outside Effect (Agar bahar click ho to band karo)
  useEffect(() => {
    const handleClickOutside = (event) => {
      // Agar menu khula hai AUR click menu ya button ke andar nahi hua
      if (
        isMobileMenuOpen &&
        menuRef.current &&
    ...
    <>
      ...
      {userInfo && userInfo.role === 'seller' && (
       ...
       {userInfo && (
           <button
             onClick={handleLogout}
             className="block w-full text-left px-4 py-2 text-red-400 hover:bg-gray-700 rounded cursor-pointer"
           >
             Logout
           </button>
       )}
  ...
  return (
    ...
    >
      <div className="container mx-auto px-4">
        <div className="flex justify-between items-center py-3">
          {/* Logo */}
          <Link to="/" className="text-2xl font-bold flex-shrink-0 cursor-pointer">🛒 SmartCart</Link>
          ...
          <div className="flex items-center space-x-4 flex-shrink-0">
            {userInfo ? (
              ...
              <>
                <Link to="/login" className="hover:text-green-200 dark:hover:text-gray-300 cursor-pointer">Login</Link>
                <Link to="/signup" className="bg-white text-green-600 font-semibold px-3 py-1 rounded hover:bg-gray-100 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 cursor-pointer">Sign Up</Link>
              </>
            )}

            {/* Hamburger Button with Ref */}
             <button 
                  ref={buttonRef} // Ref yahan lagaya
                  onClick={toggleMobileMenu} 
                  className="p-2 rounded-md focus:outline-none focus:ring-2 focus:ring-inset focus:ring-white cursor-pointer" 
                  aria-label="Toggle menu"
                >
                  <svg className="h-6 w-6" stroke="currentColor" fill="none" viewBox="0 0 24 24">
                    {isMobileMenuOpen ? (
      ...
      {/* Mobile Menu Dropdown with Ref */}
      {isMobileMenuOpen && (
        <div 
          ref={menuRef} // Ref yahan lagaya
          className="absolute top-full right-4 w-64 mt-2 bg-green-700 dark:bg-gray-800 border border-green-800 dark:border-gray-700 rounded-md shadow-lg py-2 z-50"
        >
          <div className="px-2 space-y-1">
            {renderNavLinks()} 
          </div>
